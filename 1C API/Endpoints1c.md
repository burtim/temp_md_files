...
### api/repair/history?autoId={AUTO_ID}
GET (Получить историю ремонтов автомобиля (получает полную информацию о каждом ремонте авто c детализацией!))
```json
отдает
[
  {
...
### api/repair/history?clientId={CLIENT_ID}
GET (Получить историю всех ремонтов всех автомобилей клиента без детализации (для списка))
```json
отдает
{
  "client": {
...
## LoyaltySettigs
### api/loyalty/settings?stationId={STATION_ID} 
GET //Получить настройки для СТО
```json
//отдает
{
  "percent" : 0, //Процент бонусов от покупки
  "lifeDays" : 0 //Время жизни бонусов
}
```

### api/loyalty/settings?stationId={STATION_ID} 
POST //Создать или обновить настройки для СТО
```json
//Принимает
{
  "percent" : 0, //Процент бонусов от покупки
